...
function pickJoinColor(room) {
  ...
  for (let playerIndex = roundStart; playerIndex < players.length; playerIndex++) {
    const groupIndex = colorGroupIndex(players[playerIndex].color);
  ...
  for (let groupIndex = 0; groupIndex < PLAYER_COLOR_GROUPS.length; groupIndex++) {
    ...
    const candidates = PLAYER_COLOR_GROUPS[groupIndex].filter((color) => !used.has(color));
